  ...
  return (
    <div className="space-y-5">
      <section className="rounded-3xl bg-white p-5 shadow-sm sm:p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-muted">Pantau proses komplain sampai selesai ditangani admin.</p>
      </section>
      ...
      {loading ? (
        ...
        <MarketplaceEmptyState title="Belum ada complaint" description="Complaint dari detail order akan tampil di sini." icon={<MessageSquareWarning size={28} />} />
      ) : (
        <section className="grid gap-4">
          {complaints.map((complaint) => (
            <article key={complaint.id} className="rounded-2xl border border-line bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-brand-200 hover:shadow-soft">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <h2 className="truncate text-lg font-bold text-ink">{complaint.booking?.service?.name || complaint.booking?.booking_code || `Complaint #${complaint.id}`}</h2>
                  <p className="mt-1 text-sm text-muted">Provider: {complaint.provider?.provider_profile?.business_name || complaint.provider?.name || "-"}</p>
                </div>
        ...
        {selected ? (
          <div className="space-y-5 text-sm">
            <div className="rounded-2xl bg-ink p-5 text-white">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="text-xs font-bold uppercase text-white/60">{selected.booking?.booking_code || `Complaint #${selected.id}`}</div>
                  <div className="mt-2 text-xl font-bold">{selected.booking?.service?.name || "Complaint BantuHub"}</div>
                </div>
