  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-muted">Review yang sudah diberikan untuk layanan.</p>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState title="Belum ada review" description="Review yang Anda kirim dari detail order akan tampil di sini." />
      ) : (
        <DataTable headers={["Rating", "Booking", "Service", "Provider", "Review", "Created"]}>
          {reviews.map((review) => (
            <tr key={review.id}>
              ...
              <td className="px-4 py-3 text-muted">{review.provider?.name || "-"}</td>
