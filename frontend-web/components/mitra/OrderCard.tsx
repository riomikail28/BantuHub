...
export function OrderCard({ booking, onDetail }: { booking: Booking; onDetail: (booking: Booking) => void }) {
  return (
    <article className="rounded-xl border border-line bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-brand-200 hover:shadow-soft">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h2 className="truncate text-lg font-bold text-ink">{booking.service?.name || "Pesanan BantuHub"}</h2>
          <div className="mt-2 flex items-center gap-2 text-sm text-muted">
            <UserCircle size={15} />
            <span className="truncate">Customer: <span className="font-semibold text-ink">{booking.customer?.name || "-"}</span></span>
          </div>
        </div>
      ...
      <div className="mt-5 grid gap-3 text-sm sm:grid-cols-3">
        ...
        <div className="rounded-xl border border-line bg-white px-3 py-2">
          <div className="flex items-center gap-1 text-xs font-semibold text-muted"><CalendarDays size={13} /> Jadwal</div>
          <div className="mt-1 font-bold text-ink">{formatDate(booking.booking_date)}</div>
        </div>
        <div className="rounded-xl border border-line bg-white px-3 py-2">
          <div className="text-xs font-semibold text-muted">Kode</div>
